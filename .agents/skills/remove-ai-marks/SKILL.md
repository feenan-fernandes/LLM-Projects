...
- eferences/mark-classes.md — Unicode / sampling / C2PA / containers
- eferences/vendor-notes.md — Claude, Gemini/SynthID, OpenAI, open-LLM
- eferences/removal-matrix.md — which layer when
- eferences/ethics.md — intended use
- eferences/how-claude-marks.md — Anthropic-specific detail
- eferences/markdiffusion.md — optional MarkDiffusion image harness (schemes, honesty caveats)
...
(	ext_detectors.markllm,
	ext_detectors.claude-text), and which heavy backends are configured
...
/clean and /inspect route by the uploaded 
ame extension plus the bytes;
...
fkc, ggressive_homoglyphs (text),
keep_non_ai_metadata, strip_all_metadata, emove_pixel (ctrlregen |
diffusion) (images), lso_layer_a_text (containers), deep_images
(uto | lways | lossless | 
ever, PDF: how hard to chase metadata
...
When /capabilities reports a detector (	ext_detectors.markllm) or an image
...
{"options": {"detect_before": true, "detect_after": true}} returns
	ext_detectors.before/after (text) or synthid_before/synthid_after
...
the API in Aug 2026 — see eferences/vendor-notes.md.)
...
**Code files:** Prefer formatter (prettier, lack, gofmt, …) + Layer A. Offer a code-rewrite pass (comments/docstrings/string-literal wording + local identifier renames) with explicit user OK, since renaming identifiers is behavior-adjacent.
...
Audit exit codes (same in --json, --sarif and human output):   no
...
- The "image data untouched" guarantee covers the codecs Ghostscript can pass
  ...
  exactly.
- Pixel-domain **image** watermarks can be removed optionally via the external CtrlRegen backend (emove_pixel: ctrlregen) or MarkDiffusion's DiffusionPurification (emove_pixel: diffusion); both are heavy, drift the image, and need the backend present (/capabilities). Audio/video watermarks remain out of scope.
